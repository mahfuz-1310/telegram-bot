...
# Basic Route for Render Web Service
@app.route('/')
def home():
    return "Bot is running live!"

# Data Lists
male_first_names = ['Aryan', 'Tanvir', 'Rahim', 'Sakib', 'Fahim', 'Nayeem', 'Rakib', 'Mehedi', 'Mahin', 'Sabbir', 'Arif', 'Shanto', 'Farhan', 'Niloy', 'Siam', 'Hridoy']
female_first_names = ['Sadia', 'Anika', 'Tasfia', 'Noshin', 'Faria', 'Sumaiya', 'Jannat', 'Ishrat', 'Riya', 'Muna', 'Tisha', 'Mim', 'Priya', 'Nusrat']
last_names = ['Ahmed', 'Hossain', 'Chowdhury', 'Islam', 'Khan', 'Rahman', 'Uddin', 'Talukder', 'Hasan', 'Mondol', 'Sarker', 'Biswas']
emojis = ['🔥', '✨', '👑', '😎', '💫', '🌟', '🚀', '🎯', '💯', '⚡', '💎']
...
def send_main_menu(message_or_call):
    ...
    btn_pass = types.InlineKeyboardButton('🔑 Password', callback_data='pass_12')
    btn_mail = types.InlineKeyboardButton('📧 Hotmail / Outlook', callback_data='gen_outlook_mail')
...
def handle_callback(call):
    ...
    if call.data == 'gen_male':
        ...
        name = f"{random.choice(male_first_names + female_first_names).lower()} {random.choice(last_names).lower()} 😎⚡"
    ...
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(types.InlineKeyboardButton('🔄 Generate Again', callback_data=call.data))
...
if __name__ == '__main__':
    # Bot runs in background thread so Flask can bind to port immediately
